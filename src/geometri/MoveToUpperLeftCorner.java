...
/**
 * A small animation of constructing different geometrical forms, where color
 * and postion are random. Thereafter it is possible to move all forms to the
 * upper left corner of the drawing area, i.e. moving them pixel by pixel to
 * origo. All forms are drawn in order of their size, with the biggest form
 * first and smallest last.
 * <P>
 * <tt>Point</tt> have been increased to 2 pixels in diameter and leaves a trace
 * on its way to origo. When a <tt>Point</tt> is choosen, each second time it
 * become black, which will be checked in a <tt>HashSet</tt>, in order to test
 * the methods of <tt>equals</tt> and <tt>hashCode</tt>.
 *
 * @author (Bror Bjerner/Christer Carlsson)
 * @version (nov 2014)
 */
public class MoveToUpperLeftCorner extends JFrame implements ActionListener {
	...
	private SortedSet<GeometricalForm> forms = new TreeSet<GeometricalForm>(
			new BigFirst());
	private boolean nextBlack = false;

	// Buttons for each form that can be choosen, a button to start the moving
	// and a
	...
	public MoveToUpperLeftCorner() {
		...
		/*
		 * Use this if you want to check vertical and horisontal lines try { Use
		 * this to check forms.add( new Line( 10, 10, 10, 60, new
		 * Color(50,75,200))); forms.add( new Line( 20, 20, 60, 20, new
		 * Color(50,200,100))); } catch ( IllegalPositionException ipe ) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		Color c = new Color((int) Math.round(Math.random() * 255),
				(int) Math.round(Math.random() * 255), (int) Math.round(Math
						.random() * 255));
		...
		try {
			if (e.getSource() == point) {
				...
				if (pointSet.contains(p))
					JOptionPane
							.showMessageDialog(this,
									"Point with this colour already exists, try again.");
				else {
					pointSet.add(p); // just in order to check hashCode !!
					points.add(p);
					tracePoints.add(new TracePoint(p, p.getColor()));
					JOptionPane.showMessageDialog(this,
							"A new random thick point is created");
				}
			} else if (e.getSource() == line) {
				if (forms.add(new Line(x, y, 
				...
				c)))
					...
					JOptionPane.showMessageDialog(this,
							"A new random line is created.");
				else
					JOptionPane
							.showMessageDialog(this,
									"Line: Form with this size already exists, try again");
			} else if (e.getSource() == square) {
				int length = Integer.parseInt(JOptionPane
						.showInputDialog("Choose side length (1-500):"));
				if (!forms.add(new Square(x, y, length, c)))
					JOptionPane
							.showMessageDialog(this,
									"Square: Form with this size already exists, try again");
			} else if (e.getSource() == rectangle) {
				int width = Integer.parseInt(JOptionPane
						.showInputDialog("Choose width (1-500)"));
				int heigth = Integer.parseInt(JOptionPane
						.showInputDialog("Choose heigth (1-500)"));
				if (!forms.add(new Rectangle(x, y, width, heigth, c)))
					JOptionPane
							.showMessageDialog(this,
									"Rectangle: Form with this size already exists, try again");

			} else if (e.getSource() == circle) {
				int radius = Integer.parseInt(JOptionPane
						.showInputDialog("Choose diameter (1-500)"));
				if (!forms.add(new Circle(x, y, radius, c)))
					JOptionPane
							.showMessageDialog(this,
									"Circle: Form with this size already exists, try again");
				...
				if (!forms.add(new Oval(x, y, width, heigth, c)))
					JOptionPane
							.showMessageDialog(this,
									"Oval: Form with this size already exists, try again");
	...
	private class BigFirst implements Comparator<GeometricalForm> {
		...
		public int compare(GeometricalForm f1, GeometricalForm f2) {
...
class ThickPoint extends Point {
	...
	public ThickPoint(int x, int y, Color c) throws IllegalPositionException {
		super(x, y, c);
	} // constructor Point
...
class TracePoint extends Point {
	...
	public TracePoint(int x, int y, Color c) throws IllegalPositionException {
		super(x, y, c);
	} // constructor Point
